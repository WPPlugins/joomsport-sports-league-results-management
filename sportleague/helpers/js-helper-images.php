...
class jsHelperImages
{
    public static function getEmblem($img, $type = 0, $class = '', $width = 0)
    {
        ...
        if ($img) {
            $html = '<img alt="" class="img-thumbnail img-responsive'.$class.'" src="'.$img.'" '.($width?'width="'.$width.'"':"").' />';
        } else {
            $html = '<img alt="" class="img-thumbnail img-responsive'.$class.'" src="'.JOOMSPORT_LIVE_URL_IMAGES_DEF.$defimg.'" width="'.$width.'" />';
        }
